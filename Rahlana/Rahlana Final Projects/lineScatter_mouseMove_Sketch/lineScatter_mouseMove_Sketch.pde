...
void draw() {
    ...
    if (mouseY > height/2){ 
      ...
      background(255);
      //translate(width/2, height/2);
      rotate(millis() * 0.001 * TWO_PI / 30.0);
      //line segments spacing 
    for (int i = 0; i < 360; i+=10) {
    for (int q = 0; q < 360; q+=400) {
    float x = map(sin(radians(q+i+frameCount))*i, -200, 800, i, -i);
    float y = map(cos(radians(q+i+frameCount))*i, -200, 200, -i, i);
       
    float x2 = map(sin(radians(q+i+2+frameCount))*(i+2), -200, 200, i+2, -(i+2));
    float y2 = map(cos(radians(q+i+2+frameCount))*(i+2), -200, 200, -(i+2), i+2);
